...
export default function AddressSelector() {
  const {
    ...
    street, coordinates, loading, error,
    loadProvinces, selectProvince, selectDistrict, selectWard, setStreet, geocode, autocompleteStreets
  } = useAddressStore();
  ...
  const handleStreetInputClick = () => {
    if (street.length >= 2 && selectedWard && selectedDistrict && selectedProvince) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (streetInputRef.current && !streetInputRef.current.contains(event.target)) {
        setShowSuggestions(false);
      }
    };
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-gray-50 rounded-lg p-6">
              <h3 className="text-lg font-semibold mb-4 text-gray-800">📍 Địa chỉ đã chọn:</h3>
              <div className="space-y-2 text-sm">
                <p><span className="font-medium">Tỉnh/Thành:</span> {selectedProvince?.name || "Chưa chọn"}</p>
                <p><span className="font-medium">Quận/Huyện:</span> {selectedDistrict?.name || "Chưa chọn"}</p>
                <p><span className="font-medium">Phường/Xã:</span> {selectedWard?.name || "Chưa chọn"}</p>
                <p><span className="font-medium">Đường:</span> {street || "Chưa nhập"}</p>
              </div>
            </div>
            ...
            {coordinates && (
              <div className="bg-white rounded-lg p-4 shadow-inner">
                <iframe
                  ...
                  allowFullScreen title="Map" className="rounded-lg"
                />
              </div>
            )}
          </div>
